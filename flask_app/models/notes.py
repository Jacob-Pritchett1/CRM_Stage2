...
class Post:
    ...
    def create_note(cls, data):
        query = """ INSERT INTO notes(note, created_at, updated_at, user_id)
        VALUES (%(note)s, NOW(), NOW(), %(user_id)s);"""
    ...
    def users_notes(cls, data):
        query= "SELECT * from user LEFT JOIN notes ON note.user_id = user.id WHERE user.id = %(id)s;"
